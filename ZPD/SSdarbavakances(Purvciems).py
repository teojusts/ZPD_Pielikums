...
def perform_initial_selections():
    try:
        ...
        # 3. Click the second dropdown to select and type "Purvciems"
        ...
        dropdown_element_2.send_keys("Purvciems")
...
def scrape_all_pages(start_page=1, max_pages=4):
    ...
    # Perform the initial selections (type 'r' and 'Purvciems')
    ...
    with open("Purvciems_vakances.json", "w", encoding="utf-8") as f:
...
driver = webdriver.Chrome()  # Ensure chromedriver is installed and available
all_data = scrape_all_pages(start_page=1, max_pages=4)  # Adjust the max_pages if necessary
